...
function writeRunFabric {
   ...
   # нужно здесь поставить путь до chaincode директории
   CHAINCODE_DIR=${SDIR}/../../chaincode
   ...
   echo "  run:
    ...
    volumes:
      - ./scripts:/scripts
      - ./$DATA:/$DATA
      - ${CHAINCODE_DIR}:/opt/gopath/src/github.com/hyperledger/fabric-samples
...
function writeIntermediateCA {
   ...
   then
       # Мы запускаем Root CA, затем регистрируем identity с атрибутом hf.IntermediateCA=true
       # (см. root-ca-register-ica.sh) для каждой peer организации
       local MY_PARENT_URL="http://$ROOT_CA_INT_USER:$ROOT_CA_INT_PASS@$ROOT_CA_HOST:7054"
   else
       local MY_PARENT_URL="http://$ROOT_CA_ADMIN_USER_PASS@$ROOT_CA_HOST:7054"
   fi
   echo "  $INT_CA_NAME:
