...
function main {
   ...
   peer chaincode instantiate -C $CHANNEL_NAME -n mycc -v 1.0 -c '{"function":"initLedger","Args":[""]}' -P "$POLICY" $ORDERER_CONN_ARGS -l "node"
   ...
   # Query chaincode from the 1st peer of the 2nd org
   initPeerVars ${PORGS[1]} 1
   switchToUserIdentity
   #chaincodeQuery 100

   # Query chaincode from the 1st peer of the 3rd org
   ...
   # Query chaincode on 1nd peer of 1st org
   #sleep 10
   #initPeerVars ${PORGS[0]} 1
   #switchToUserIdentity
   #chaincodeQuery 90
...
function installChaincode {
   ...
   # здесь путь до директории захардкожен, см. makeDocker.sh
   # (nodejs)
   peer chaincode install -n mycc -v 1.0 -p /opt/gopath/src/github.com/hyperledger/fabric-samples/objects-ledger/node -l "node"
}
...
function createConfigUpdatePayloadWithCRL {
   ...
   # у нас есть некоторый directory stack
   # находясь в текущей директории, мы загоняем в стек /tmp
   pushd /tmp
   ...
   # возвращаемся на ту директорию, которая на верхушке стека
   # а какая эта директория? (/tmp?)
   popd
}
