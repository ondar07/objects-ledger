...
# Set to true to populate (заполнить) the "admincerts" folder of MSPs
ADMINCERTS=true
...
# initOrgVars <ORG>
# эта функция инициализирует все соответствующие переменные, которые
# будут относиться к <ORG>
# например, $ORG=org0 (если <ORG>=org0)
#           или $ROOT_CA_ADMIN_USER=rca-org0-admin и т.д.
function initOrgVars {
   ...
   # здесь захардкожены названия peer организаций
   if [[ $ORG = "org1" || $ORG = "org2" || $ORG = "org3" ]]
   then
        # this is for root ca and ica sync
        ROOT_CA_SUCCESS_FILE=/$DATA/rca-success-file
        ROOT_CA_HOST=rca
        ROOT_CA_NAME=rca
        ROOT_CA_ADMIN_USER=rca-admin
        ROOT_CA_CERTFILE=/${DATA}/rca-cert.pem
   else
...
function switchToAdminIdentity {
   ...
   if [ ! -d $ORG_ADMIN_HOME ]; then
       # если НЕ существует директории $ORG_ADMIN_HOME
       # то создаем такую, попутно за-enroll-ив админа (обычного; admin-org1, например)
       # иначе (см. внизу) просто export CORE_PEER_MSPCONFIGPATH
       # т.е. просто экспортируем путь до (local) msp админа
...
function switchToUserIdentity {
    # здесь будет использоваться user директория
    # т.к. у нас для каждого org есть только 1 user, то норм
    # но вообще, правильнее наверно $ORG/$USER_NAME
...
function finishMSPSetup {
	echo "finishMSPSetup is not used!"
}
